...
export default function Footer() {
  ...
  return (
    <footer className="relative w-full bg-[#0a0f0a] text-white border-t border-white/10 overflow-hidden select-none">
      {/* top accent */}
      ...
      <div className="relative z-10 w-full max-w-[85.5%] mx-auto px-0 py-14">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-10">
          ...
          <div className="flex flex-col gap-4">
            <Link
              href="/"
              className="text-white font-black text-h2 tracking-wider uppercase w-fit"
            >
              TERRAMORE
            </Link>
            <p className="text-white/70 text-body font-normal">
          ...
          {/* Navegación */}
          <div className="flex flex-col gap-4">
            <h4 className="text-white font-bold text-label uppercase tracking-widest mb-1">
              Navegación
            </h4>
        ...
        <div className="max-w-[85.5%] mx-auto px-0 py-6 flex flex-wrap items-center justify-center lg:justify-between gap-x-10 gap-y-3 text-center">
          {["REPROCANN · Min. de Salud", "Estudio de Abogados Robles", "Laboratorio propio ArMedic"].map(
            (item) => (
              <span
                key={item}
                className="text-label font-bold uppercase tracking-widest text-white/65"
              >
                {item}
              </span>
            ),
